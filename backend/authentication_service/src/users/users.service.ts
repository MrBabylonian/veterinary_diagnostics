...
export class UsersService {
  private users: User[] = [
    {
      ...
      last_name: "Doe",
      password_hash:
        "$argon2id$v=19$m=65536,t=3,p=4$hLlYEyE9IaWYOR0Fy1iSPA$mTA7lMqBOhh2yCsfRBrMqEp0uBD2XiYrSGcl7CgEblM",
      auth_provider: "local",
  ...
  // TODO: Make sure getters are returning UserForPublicDto with plainToClass after implementing the database
  ...
  findByEmail(email: string): UserForPublicDto {
    ...
    return plainToClass(UserForPublicDto, user, {
  ...
  create(userToCreate: User) {
    ...
    const registeredUser = plainToClass(UserForPublicDto, userToCreate, {
      excludeExtraneousValues: true,
    });
    return {
      status: 201,
